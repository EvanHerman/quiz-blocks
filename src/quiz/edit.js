		...
		<div {...useBlockProps()}>
			...
			{( quizzes && ! quizzes.length ) &&
				<div id="quiz-blocks-quiz" className="create-quiz">
					<strong>{__( 'No Quizzes Found', 'quiz-blocks' )}</strong>
					<p><a href={quizBlocksQuiz.createQuizURL}>{__( 'Create a quiz', 'quiz-blocks' )}</a>.</p>
				</div>
			}
			{( quizzes && 0 !== quizzes.length && 0 === attributes.quizID ) &&
				<div id="quiz-blocks-quiz" className="select-quiz">
					<strong>{__('Select a Quiz', 'quiz-blocks')}</strong>
					<p>{selectQuizText}</p>
					{ isSelected &&
						<SelectControl
							value={attributes.quizID}
