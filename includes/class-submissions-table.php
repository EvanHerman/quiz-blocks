						...
						<div class="meta-box-sortables">
							...
							<div class="postbox">
								...
								<div class="inside">
									...
									<a
										class="button button-secondary delete"
										<?php if ( empty( $this->submissions ) ) { ?>
										disabled="disabled"
										<?php } ?>
										onclick="<?php if ( empty( $this->submissions ) ) { ?>
										return false;
										<?php } else { ?>
										return confirm( '<?php printf( /* translators: %s is the users display name. */ esc_attr__( 'Are you sure you want to delete all %s submissions? This cannot be undone.', 'quiz-blocks' ), esc_attr( get_the_title( $this->quiz_id ) ) ); ?>' )
										<?php } ?>"
										href="<?php echo esc_url( $delete_all_submissions_url ); ?>"
									>
